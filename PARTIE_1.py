...
class Client:
    ...
    class Compte:
        ...
        def supprimer_compte(self, num_compte):
            ...
            if num_compte in self.select_num_compte():
                cur.execute('DELETE FROM Compte_table WHERE num_compte=?', (num_compte,))
                connection.commit()
        ...
        class Carte:
            ...
            def supprimer_carte(self, num_carte):
                ...
                if num_carte in self.select_num_carte():
                    cur.execute('DELETE FROM Carte_table WHERE num_carte=?', (num_carte,))
                    connection.commit()
    ...
    def default_num(self):
        ...
        while self.r1==True:
            ...
            if len(self.class_carte.select_num_carte()) == 10:
                print('toutes les numéros des cartes sont utilisé ')
                break
        self.r2 = True
        while self.r2 == True:
    ...
    def depart(self):
            ...
            while a == True:
                reponse = input("#####################################\n"
                                ...
                                "------------------------------------------ \n"
                                "Pour supprimer une carte répondre par 11 \n"
                                "------------------------------------------ \n"
                ...
                if reponse == "3":
                    ...
                    code_secret = input("entrer le code secret de cette carte : ")
                ...
                if reponse=="4":
                    ...
                    print("\nVotre solde a été modifié avec succès \n")

                if reponse=="5":
                    solde = input("entrer le montant que vous voulez ajouter: ")
                    num_compte = input("entrer le numéro de compte que vous voulez modifier: ")
                    self.class_compte.crediter_solde(solde, num_compte)
                    print("\nVotre montant a été modifié avec succès \n")
                ...
                if reponse == "6":
                    ...
                    if num_compte in self.class_compte.select_num_compte():
                        ...
                        print("le solde de compte ",num_compte," est : ",self.class_compte.consulter(num_compte), "DA\n")
                    else:
                ...
                if reponse == "9":
                    ...
                    print("\nVotre carte a été débloqué \n")

                if reponse == "10":
                    num_carte = input("entrer le numéro de carte que vous voulez changer son code secret: ")
                    nouvelle_code_secret = input("entrer le nouveau code secret: ")
                    self.class_carte.modifier_code_secret(nouvelle_code_secret,num_carte)
                    print("\nVotre code secret a été changé avec succès \n")
                if reponse=="11":
                    num_carte=input("entrer le numéro de carte que vous voulez supprimer: ")
                    self.class_carte.supprimer_carte(num_carte)
